...
describe("RequestEditPage tests", () => {
  ...
  afterEach(() => {
    axiosMock.reset(); // Reset after each test to avoid conflicts.
  });
  ...
  describe("tests where backend is working normally", () => {
    ...
    test("Is populated with the data provided 2", async () => {
      axiosMock.reset();
      axiosMock.resetHistory();
      axiosMock
        .onGet("/api/currentUser")
    ...
    test("Changes when you click Update 2", async () => {
      axiosMock.reset();
      axiosMock.resetHistory();
      axiosMock
        .onGet("/api/currentUser")
      ...
      await waitFor(() => expect(mockToast).toBeCalled());
      expect(mockToast).toBeCalledWith(
        "Request Updated - id: 17 professor name: testnameother professorEmail: testnameotheremail@ucsb.edu requester name: testname2 recommendation: PhD program details: test_details",
      );
      expect(mockNavigate).toBeCalledWith({ to: "/requests/statistics" });
    });
  });
